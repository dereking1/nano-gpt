...
if args.mode == 'pretrain':
    ...
    ############# YOUR CODE HERE #############
    trainer_cfg = trainer.TrainerConfig(max_epochs=650,
                                        batch_size=128,
                                        learning_rate=args.pretrain_lr,
                                        lr_decay=True,
                                        warmup_tokens=512*20,
                                        final_tokens=200*len(pretrain_dataset)*block_size,
                                        num_workers=4)
    gpt_trainer = trainer.Trainer(gpt, pretrain_dataset, None, trainer_cfg)
    gpt_trainer.train()
    ...
    if args.reading_params_path is not None:
        ...
        max_epochs = 10
    trainer_cfg = trainer.TrainerConfig(max_epochs=max_epochs,
                                        batch_size=256,
                                        learning_rate=args.finetune_lr,
                                        lr_decay=True,
                                        warmup_tokens=512*20,
    ...
    evaluate_dataset = dataset.NameDataset(
        pretrain_dataset,
        open(args.evaluate_corpus_path, encoding='utf8').read()
    )
    gpt_trainer = trainer.Trainer(gpt, finetune_dataset, evaluate_dataset, trainer_cfg)
    gpt_trainer.train()
    torch.save(gpt.state_dict(), args.writing_params_path)
    ##########################################
