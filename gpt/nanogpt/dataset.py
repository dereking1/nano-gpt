...
class WikiDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.char_corruption:
            ...
            input_string = torch.tensor([self.stoi[s] for s in x], dtype=torch.long)
            output_string = torch.tensor([self.stoi[s] for s in y], dtype=torch.long)
